...
export default function QuoteManager(){
 ...
 if(loading)return <div className="general-settings-state"><RefreshCw className="settings-spin" size={20}/><div><strong>جاري تحميل العبارات</strong><span>يرجى الانتظار قليلًا</span></div></div>;
 return <div>
  <section className="settings-panel-intro"><span className="settings-panel-intro__icon"><Quote size={21}/></span><div><small>محتوى شاشة العرض</small><h3>العبارات التحفيزية</h3><p>أضف وعدّل العبارات التي تظهر على شاشة العرض.</p></div><span className="real-badge"><CheckCircle2 size={13}/> {quotes.length} عبارة</span></section>
  <section className="settings-real-card">
   ...
   <div className="quote-add-pro"><input className="quote-editor-input" value={newQuote} onChange={e=>setNewQuote(e.target.value)} onKeyDown={e=>e.key==="Enter"&&add()} placeholder="أضف عبارة تحفيزية جديدة"/><button type="button" className="btn-primary" disabled={!newQuote.trim()||adding} onClick={add}><Plus size={14}/> إضافة</button></div>
  </section>
...
function QuoteRow({item,onSave,onDelete}){const [value,setValue]=useState(item.quote||"");return <div className="quote-row-pro"><input className="quote-editor-input" value={value} onChange={e=>setValue(e.target.value)}/><div className="quote-actions-pro"><button type="button" className="icon-btn" title="حفظ" onClick={()=>onSave(item.id,value)}><Save size={15}/></button><button type="button" className="icon-btn danger" title="حذف" onClick={()=>onDelete(item.id)}><Trash2 size={15}/></button></div></div>}
